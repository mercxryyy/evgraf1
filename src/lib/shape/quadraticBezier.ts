...
export class QuadraticBezier extends Shape {
    private _points: { x: number; y: number }[] = []; // [p0, p1, p2]

    constructor(p0: { x: number; y: number }, p1: { x: number; y: number }, p2: { x: number; y: number }, id?: string) {
        super(id);
        this._points = [p0, p1, p2];
        this.recenter();
    }
    ...
    clone(): QuadraticBezier {
        const qb = new QuadraticBezier({x:0,y:0}, {x:0,y:0}, {x:0,y:0}, this.id);
        qb._points = this._points.map(p => ({ ...p }));
        qb.transform.copyFrom(this.transform);
        qb.strokeStyle = this.strokeStyle;
        qb.strokeWidth = this.strokeWidth;
        qb.strokeOpacity = this.strokeOpacity;
        return qb;
    }
}